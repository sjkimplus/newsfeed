    ...
    public User findTypeItemId(AlarmTypeEnum type, Long itemId) {
        return switch (type) {
            case COMMENT -> userRepository.findById(postCommentRepository.findById(itemId)
                            .orElseThrow(() -> new NullPointerException("해당 ID의 댓글이 없습니다.")).getUserId())
                    .orElseThrow(() -> new NullPointerException("해당 ID의 유저가 없습니다."));
            case LIKE -> likeRepository.findById(itemId)
                    .orElseThrow(() -> new NullPointerException("해당 ID의 좋아요가 없습니다.")).getUser();
            case RELATIONSHIP -> relationshipRepository.findById(itemId)
                    .orElseThrow(() -> new NullPointerException("해당 ID의 친구 요청이 없습니다.")).getSentUser();
        };
    }
